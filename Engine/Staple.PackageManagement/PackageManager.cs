...
public partial class PackageManager
{
    ...
    public string BuiltinPackagesPath => Path.Combine(StorageUtils.EditorPath.Value, "Packages");
    ...
    public void AddPackage(string name, string version, Action refreshAssets, Action resetAssetPaths)
    {
    ...
    public void RemovePackage(string name, Action refreshAssets, Action resetAssetPaths)
    {
        var packageList = ParsePackages(PackagesPath);
    ...
    public void InstallGitPackage(string url, Action refreshAssets, Action resetAssetPaths)
    {
    ...
    public void InstallLocalPackage(string path, Action refreshAssets, Action resetAssetPaths)
    {
    ...
    public bool Refresh(Action resetAssetPaths)
    {
        StorageUtils.CreateDirectory(PackagesCacheDirectory);
        ...
        if (version?.Success ?? false)
        {
            ...
            if(builtinPackages.TryGetValue(name, out var p))
            {
                ...
                if (Directory.Exists(directory) == false)
                {
                    try
                    {
                        StorageUtils.CopyDirectory(p.Item1, directory);
                    }
                    catch (Exception e)
                    {
    ...
    private bool TryGitClone(string url, out Package package, out string path)
    {
        ...
        StorageUtils.CreateDirectory(path);
        ...
        if(pathOption.Length > 0)
        {
            try
            {
                StorageUtils.DeleteDirectory(Path.Combine(path, "..", "STAPLE_STAGING"));
                ...
                foreach(var d in directories)
                {
                    ...
                    StorageUtils.DeleteDirectory(d);
                }
                ...
                StorageUtils.CopyDirectory(Path.Combine(path, pathOption), Path.Combine(path, "..", "STAPLE_STAGING"));

                StorageUtils.DeleteDirectory(Path.Combine(path, pathOption));

                StorageUtils.CopyDirectory(Path.Combine(path, "..", "STAPLE_STAGING"), Path.Combine(path));
